...
namespace ACNN
{
	...
	int Pooling::forward(const std::vector<aMat>& bottom_blobs, std::vector<aMat>& top_blobs, const Option& opt) const
	{
		...
		{
			ACNN_LOGE("Pooling ERROR: bottom_blobs size(%d) != top_blobs size(%d)", (int)bottom_blobs.size(), (int)top_blobs.size());
			return -1;
		}
		...
		for (size_t i = 0; i < bottom_blobs.size(); i++)
		{
			...
			{
				ACNN_LOGE("Pooling Type is not supported: %d, %d", adaptive_pooling, pooling_type);
				return -1;
			}
			else
			{
