...
namespace ACNN
{
	...
	int ParamDict::load_param(const DataReader& dr)
	{
		...
		while (dr->scan("%d=", &id) == 1)
		{
			...
			{
				ACNN_LOGE("id < NCNN_MAX_PARAM_COUNT failed (id=%d, NCNN_MAX_PARAM_COUNT=%d)", id, ACNN_MAX_PARAM_COUNT);
				return -1;
			}
			...
			if (is_array)
			{
				...
				if (nscan != 1)
				{
					ACNN_LOGE("ParamDict read array length failed");
					return -1;
				}
				...
				for (int j = 0; j < len; j++)
				{
					...
					if (nscan != 1)
					{
						ACNN_LOGE("ParamDict read array element failed");
						return -1;
					}
					...
					else
					{
						...
						if (nscan != 1)
						{
							ACNN_LOGE("ParamDict parse array element failed");
							return -1;
						}
					}
			...
			else
			{
				...
				if (nscan != 1)
				{
					ACNN_LOGE("ParamDict read value failed");
					return -1;
				}
				...
				else
				{
					...
					if (nscan != 1)
					{
						ACNN_LOGE("ParamDict parse value failed");
						return -1;
					}
				}
