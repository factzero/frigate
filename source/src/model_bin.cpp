...
namespace ACNN
{
	...
	aMat ModelBinFromDataReader::load(int w, int type) const
	{
		...
		if (type == 0)
		{
			...
			{
				ACNN_LOGE("ModelBin read flag_struct failed %zd", nread);
				return aMat();
			}
			...
			{
				// half-precision data
				ACNN_LOGE("ModelBin load half-precision data not supported");
				return aMat();
			}
			else if (flag_struct.tag == 0x000D4B38)
			{
				// int8 data
				ACNN_LOGE("ModelBin load int8 data not supported");
				return aMat();
			}
			else if (flag_struct.tag == 0x0002C056)
			{
				...
				{
					ACNN_LOGE("ModelBin read weight_data failed %zd", nread);
					return aMat();
				}
			...
			{
				// quantized data
				ACNN_LOGE("ModelBin load quantized data not supported");
				return aMat();
			}
		...
		{
			ACNN_LOGE("ModelBin load type %d not implemented", type);
			return aMat();
		}
